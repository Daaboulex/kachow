---
description: >
  One-command sync of ALL context artifacts between Claude and Gemini — hooks, skills,
...
memory, or command to ensure Claude ↔ Gemini parity across all locations.

## What it syncs

| Artifact | Direction | How |
|---|---|---|
| Skills | Claude → Gemini ([user]) | Copy .claude/skills/ → .gemini/skills/ |
| Rules | Claude → Gemini ([user]) | Copy .claude/rules/ → .gemini/rules/ |
| Memories | Claude global → Gemini project | Copy new/updated files |
| Commands | ~/.claude/commands/ → ~/.gemini/commands/ | Copy with Claude→Gemini adaptations |
| CLAUDE.md → GEMINI.md | Automatic | sync-gemini-md.js hook handles this |
| Hook files | Claude → Gemini | Copy shared hooks (skip platform-specific) |
...
- After creating a new hook → run Step 2 + register in BOTH settings.json
- After creating/editing a skill → Step 1 handles it (+ Step 3 for multi-project)
- After creating/editing a memory → Step 1 handles it
