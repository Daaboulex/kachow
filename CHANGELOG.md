...
### Added
- **agent-dependency-guard.js** — PreToolUse hook blocks Agent dispatches referencing non-existent files. Env var escape hatch `AGENT_ALLOW_MISSING_INPUTS=1`
- **cmd-deep-research** — skill formalizing investigation-to-spec workflow (understand → dispatch agents → consolidate → spec)
- **R-RES-3** (AGENTS.md) — agent dependency check before multi-agent dispatch
- **R-RES-4** (AGENTS.md) — full agent pre-dispatch protocol (8-point checklist)
...
### Fixed
- **version-change-detector crash** — removed dangling `signals` variable references (session-start-combined.js:558, 572-573)
- **session-health-fast.js:97** — `entry.ts` → `entry.timestamp` (24h violation count was inflated — NaN comparison never fired cutoff)
...
### Removed
- Redundant `CLAUDE_CODE_EFFORT_LEVEL` env var (superseded by `effortLevel` settings key)
- Orphaned `mcp__git`, `mcp__fetch`, `mcp__memory` permission allows

### Changed
- Status line redesign: `opus-4.6 · 92% ctx · xh ⚡` (minimal percentage style)
- Source repo now receives git tags alongside mirror (publish.sh fix)
