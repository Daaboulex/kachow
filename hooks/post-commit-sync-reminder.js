...
function findProjectRoot(dir) {
  // Walk up to find a directory with Portable-Builder/git/sync-repositories.ps1
  const home = require('os').homedir();
  let d = dir;
  while (d.length >= home.length) {
    const syncScript = path.join(d, 'Portable-Builder', 'git', 'sync-repositories.ps1');
...
try {
  ...
  process.stdout.write(JSON.stringify({
    continue: true,
    systemMessage: 'Commit done. Remember: this repo uses dual-remote sync (SSD + Server). Run `pwsh Portable-Builder/git/sync-repositories.ps1 -Interactive` to persist across machines.'
  }));
} catch (e) {
