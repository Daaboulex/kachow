...
function run(cmd, cwd) {
  try {
    ...
    try { process.stderr.write(`[skill-auto-updater] run failed: ${cmd.slice(0, 60)} → ${(e.stderr || e.message || 'unknown').slice(0, 150)}\n`); } catch {}
    return null;
  }
}
...
try {
  ...
  if (fs.existsSync(codexSkills)) {
    ...
    const PORTABLE_OTHER = [
      'debt-tracker', 'excalidraw', 'handoff', 'verification-before-completion',
    ];
    const PORTABLE_GSD = [
