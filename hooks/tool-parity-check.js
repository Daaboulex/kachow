#!/usr/bin/env node
// tool-parity-check.js — SessionStart hook
// Detects hook registration drift between Claude, Gemini, Codex, Crush, OpenCode.
...
const cooldownFile = path.join(toolHomeDir(), 'cache', 'tool-parity-last.json');
...
try {
        ...
        if (Date.now() - (cache.last_run || 0) < COOLDOWN_MS) {
          if (cache.warnings?.length) {
            process.stdout.write(JSON.stringify({
              continue: true,
              systemMessage: `[tool-parity] ${cache.warnings.join(' | ')}`
            }));
            process.exit(0);
          }
          passthrough();
  ...
  if (warnings.length > 0) {
    process.stdout.write(JSON.stringify({
      continue: true,
      systemMessage: `[tool-parity] ${warnings.join(' | ')}`
    }));
  } else {
    passthrough();
  }
} catch (e) {
  try { process.stderr.write('tool-parity-check: ' + e.message + '\n'); } catch {}
  passthrough();
}
