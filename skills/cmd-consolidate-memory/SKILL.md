...
If invoked as `/consolidate-memory quick` or with arg `quick`:
- Run ONLY Phases 1-2 (memory merge/dedupe + index rebuild)
- Skip Phases 3-6 (semantic synthesis, cross-checks, hook health, report)
- Minimal token usage, routine maintenance

If invoked as `/consolidate-memory deep` or `/consolidate-memory` (no arg):
- Run all 6 phases (full pipeline)
- Thorough maintenance — semantic synthesis, cross-checks, hook health

Default: quick (saves tokens for routine sessions). Use deep for periodic maintenance or when dream-auto triggers.
...
```bash
node -e "
const le = require('$HOME/.ai-context/hooks/lib/leader-election.js');
const t3 = require('$HOME/.ai-context/hooks/lib/tier3-consolidation.js');
...
node -e "
const t3 = require('$HOME/.ai-context/hooks/lib/tier3-consolidation.js');
const { readEvents, HOSTNAME } = require('$HOME/.ai-context/hooks/lib/observability-logger.js');
const os = require('os');
const memoryDir = '<memory_dir>';  // Replace with actual memory dir
...
**Step 3b: Surface Action Items to Deferred Queue**

After writing semantic files, scan them for HIGH-priority action items — lines containing `ACTION:`, `FIX:`, `MISSING:`, `REGRESSION:`, or similar urgent patterns. For each actionable finding, add it to the deferred items queue so it surfaces at SessionStart:

```bash
node -e "
const { addItem } = require('$HOME/.ai-context/hooks/lib/deferred-items.js');
// For each HIGH-priority finding from semantic analysis:
// addItem({ title: 'finding summary', type: 'ai-actionable', project_key: 'dot-ai-context' });
console.log('Action items surfaced to deferred queue');
"
```

This prevents actionable tier3 findings from being trapped in semantic files where they're invisible until the next consolidation run.
...
```bash
node -e "
const { logEvent, HOSTNAME } = require('$HOME/.ai-context/hooks/lib/observability-logger.js');
const cwd = process.cwd();
logEvent(cwd, {
...
```bash
# Reset session counter (shared ~/.ai-context/ — v0.9.5, was per-tool)
echo "0" > ~/.ai-context/.dream-session-count
# Reset cooldown timer
touch ~/.ai-context/.dream-last
# Release lock
rm -f ~/.ai-context/.dream-lock
```
