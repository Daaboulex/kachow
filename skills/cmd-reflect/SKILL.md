---
name: cmd-reflect
description: "Reflect on session, manage auto-reflect setting. Args - on, off, status, or empty for full reflection"
---
...
```bash
touch ~/.ai-context/.reflect-enabled
```
...
```bash
rm -f ~/.ai-context/.reflect-enabled
```
...
```bash
if [ -f ~/.ai-context/.reflect-enabled ]; then
...
ls .ai-context/skills/ .ai-context/rules/ .ai-context/memory/ 2>/dev/null
...
|-------|--------|--------|-------|
| **Global** | `~/.ai-context/AGENTS.md`, `~/.ai-context/commands/`, `~/.ai-context/hooks/` | `~/.gemini/extensions/` | All projects |
| **Project** | `.ai-context/skills/` (via project-state), `.ai-context/rules/` (via project-state), `.ai-context/memory/` (via project-state) | `.gemini/skills/`, `.gemini/rules/`, `.gemini/memory/` | This repo |
| **NixOS variant** | `.ai-context/` (symlinked) | `.ai-context/` (symlinked) | This repo |
...
### Q1: Already covered?
Check AGENTS.md hard rules AND `.ai-context/rules/*.md` AND existing memories.
...
→ **HOOK** (`~/.ai-context/hooks/<name>.js` + register in `settings.json`)
Examples:
- "AI-tasks.json sync" — no longer needed, tool-specific state → PostToolUse hook
...
→ **SKILL** (`project-state/<project>/skills/<name>/SKILL.md`) — loaded when description matches user intent
...
→ **RULE** (`project-state/<project>/rules/<scope>.md`) — path-scoped, always loaded for matching files
...
```bash
ls .ai-context/rules/ 2>/dev/null
```
...
|-------|---------------|-------------|
| **All projects, all time** | `~/.ai-context/AGENTS.md (global instructions) | Universal workflow rules, tool philosophy |
| **All projects, contextual** | `~/.ai-context/memory/ (global memory) | Project-specific learnings for Claude |
| **This project, all tools** | `.ai-context/rules/` (via project-state) or `.ai-context/skills/` (via project-state) + sync to `.gemini/` | Code patterns, domain skills |
| **This project, memory** | `.ai-context/memory/` (via project-state) + copy to `.gemini/memory/` | Context that guides behavior |
| **NixOS project** | `.ai-context/` (symlinked) | Same as above, NixOS layout |
...
```bash
cat ~/.ai-context/skill-lineage.json 2>/dev/null | head -5  # Check if lineage exists
```
...
If `~/.ai-context/skill-lineage.json` doesn't exist, skip this phase silently.
...
### Skill sync details
When creating or updating a skill, it must exist in BOTH `.ai-context/skills/` (via project-state) and `.gemini/skills/`.
...
```bash
# Skills shared via project-state symlinks — no manual copy needed
```
...
- Tool parity: shared via project-state symlinks (Codex/Crush/OpenCode read via symlinks)
- Report: "Context sync: OK" or "Context sync: DRIFT — [details]"

## What NOT to Save

- Rules already in CLAUDE.md or `.ai-context/rules/` (via project-state) — check Q1 FIRST
- Code patterns derivable from reading the codebase
- Git history (use `git log`)
- Debugging solutions (the fix is in the code)
- Ephemeral task state (use AI-tasks.json)
...
- `~/.claude/`, `~/.gemini/`, `~/.codex/`, `~/.crush/`, `~/.opencode/` are derived state (symlinks), NOT git repos
...
- Skills and rules live in project-state, symlinked to all tool dirs.
